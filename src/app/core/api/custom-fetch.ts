...
export const customFetch = <T>(config: CustomFetchConfig, http: HttpClient): Observable<T> => {
  ...
  // Cria os HttpParams diretamente tirando partido da imutabilidade correta através do reduce
  const httpParams = Object.entries(config.params ?? {}).reduce((acc, [key, value]) => {
    if (value === null || value === undefined) return acc;

    if (Array.isArray(value)) {
      return value.reduce((arrayAcc, arrayValue) => arrayAcc.append(key, String(arrayValue)), acc);
    }
